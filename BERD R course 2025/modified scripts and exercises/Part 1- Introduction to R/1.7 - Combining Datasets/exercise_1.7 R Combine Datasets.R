...
require(janitor)
df_mod <- clean_names(df) %>% select(dumpster, month)
df_may <- filter(df_mod, month == "May")
df_june <- filter(df_mod, month == "June")

# Question 2: Stack the two dataframes for Question 1 together. 
df_mayjune <- rbind(df_may, df_june)
...
data_combo <- merge(simul_dat, df_mayjune, by = "dumpster", all = TRUE)
...
dat = read.csv("trashwheel2014.csv")
...
trashwheel_dat = read.csv("trashwheel2014.csv") %>% 
