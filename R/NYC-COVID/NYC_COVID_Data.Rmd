...
rawData <- read_csv(file.path(datafolder,wastewaterFiles[1]), col_types = cols(Date = col_date("%m/%d/%y"), Conc = col_double()), skip = 2) %>% `colnames<-`(c("Date", sewershedIDs[1]))

# TODO: Use apply functions to iterate through the file list and make a df for each sewershed
