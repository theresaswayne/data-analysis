...
df_orig <- airquality
df <- clean_names(airquality)

# think there is a typo here; they meant 70 not 101
...
# Approach:
# 1. make ozone and wind into binary factors (or chr works too I guess)
# 2. try chi squared

df <- df %>% mutate(wind_bin = ifelse(wind < 13, "<13mph", ">=13mph"),
                    ozone_bin = ifelse(ozone < 70, "<70ppb", ">=70ppb")) # now a character

# does the data table suggest there could be a difference?
df %>%
  tabyl(ozone_bin, wind_bin, show_missing_levels = FALSE) %>%
  #adorn_totals("col") %>%                                    # Unselect this if you want col percentages
  adorn_percentages("row") %>%
  adorn_pct_formatting(digits = 1) %>%
  adorn_ns %>%
  adorn_title
# suggestive, but there are few low ozone days...

# try chisq anyway
chisq.test(df$ozone_bin, df$wind_bin, correct = F)
# get a warning
chisq.test(df$ozone_bin, df$wind_bin, correct = T)
# still get a warning
fisher.test(df$wind_bin, df$ozone_bin)
# p = 0.04

# Question 2: Does the association found in Question 1 remain the same after adjusting for temperature and solar radiation?
# still not sure what this means -- inspecting the solutions suggests that you just need to incorporate those other factors into your model and then yuo can see the effect of the factor of interest independent of the others
...
# looks like temp is sig and the others not??
s